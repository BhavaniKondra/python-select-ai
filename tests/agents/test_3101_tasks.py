...
def test_3110_fetch_non_existent_task():
    ...
    logger.info("Fetching non-existent task")
...
def test_3116_missing_instruction():
    ...
    logger.info("Creating task with missing instruction")
...
def test_3117_delete_enabled_task_without_force_succeeds():
    ...
    logger.info("Creating and deleting enabled task")
...
def test_3118_delete_disabled_task_with_force_succeeds():
    ...
    fetched = Task.fetch(task.task_name)
    assert fetched.task_name == task.task_name
    task.delete(force=True)
    expect_oracle_error("NOT_FOUND", lambda: Task.fetch(task.task_name))
